...
神器优先级也用于分配已支持的方向增伤、法术冷却恢复、减耗和行星强化。整理会尽量保留其他加成，优先满足你设置的神器和连招目标。
整理面板可切换是否允许升级后增加耗蓝，默认不允许。手动指定神器等级时，以指定等级为准。
搜索有预算限制，不保证找到最优布局或最高战斗伤害。

## 功能

- **战斗显示**：伤害、DPS、战后统计、连续命中、资源数值和敌友轮廓。
- **效果属性**：角色菜单的特殊属性页默认显示简短结果，鼠标悬停或选中条目后，通过游戏提示框查看加成与触发条件。包含太阳剑伤害与库存、乌云伤害与补充、快捷栏魔法耗蓝与充能、已支持冰霜武具按当前魔力计算的伤害、行星单弹伤害，以及灼伤和触电参考值。伤害未计入命中结算，异常状态参考值仍需相应施加来源。切页沿用游戏绑定，新增行支持随焦点滚动。目前不覆盖所有道具和效果。

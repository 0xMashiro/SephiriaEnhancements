...
internal static class ResourceBarValueChecks
{
    internal static void Run()
    {
        ...
        foreach (string language in new[] { "zh-CN", "zh-TW", "unsupported-language" })
        {
            ...
            if (texts.Count != 22 || !texts.Keys.ToHashSet().SetEquals(english.Keys) || texts.Values.Any(string.IsNullOrWhiteSpace))
                throw new InvalidOperationException("Resource bar settings require ten label/help pairs and on/off text");

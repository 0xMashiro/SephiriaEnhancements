...
打开背包后按 `F8` 即可自动整理，不必先设置目标。可选的神器优先队列默认使用自动等级目标，
无需逐槽填写等级。普通神器以有效等级上限为目标；升级会加重直接负面属性时，自动目标会受到限制。

- 将神器放入六槽优先队列或“尽量不启用”区。点击或拖动标记换位，右键移除标记。
  排除的物品仍保留在背包中。
- 指向或选中神器标记，使用当前“切换索敌”绑定打开设置。可选择自动、只需生效或指定等级；
  模式、等级和软硬条件跟随神器，换位时保留。指定更高等级可以主动接受该神器的直接属性代价。
- 规则默认“尽力满足”（Soft）。神器目标、排除项和羁绊条件可分别切换为“必须满足”（Hard），
  以 `!` 标识。羁绊在独立目标页编辑：优先 0 表示不要求最低计数，避免 N 表示计数不超过 N。
  羁绊规则跨局保留，神器标记仅用于当前探索。

Mod 求解器先要求最终布局满足全部 Hard，再依次比较 Soft 排除项、神器第 1～6 槽、其他手动目标
和默认偏好。后序目标的累计收益不能抵消前序目标的退步；前序 Soft 未满时，仍会在保留已达成进度
的前提下优化后序目标。目标完成程度相同时，优先保留已建模的收益；两案都存在取舍时，尽量少移动、少旋转。

结果核验后，颜色表示要求完成情况：**绿**为满足、**黄**为 Soft 部分满足、**红**为未满足；Hard
未达标直接显示红色。**灰色**表示尚无有效核验结果。指向或选中标记可查看当前值和目标值；目标、背包
或场景变化会清除旧颜色。颜色不代表战斗伤害提升。

找不到满足全部 Hard 的结果时不应用布局，并区分“已证明无法同时满足”和“本次搜索未找到”。
应用通过游戏正常操作执行，每次移动或旋转后都会核对整个布局及已建模的结算。发现意外变化时停止
后续操作，已完成的移动不会自动撤销。Hard 约束最终布局，不保证每个中间交换状态都满足。

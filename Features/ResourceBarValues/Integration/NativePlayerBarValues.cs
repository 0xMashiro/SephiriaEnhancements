...
namespace SephiriaEnhancements.ResourceBarValues.Integration
{
    ...
    internal static class NativePlayerBarValuesPatch
    {
        ...
        private static void PostfixCore(UI_MultiplayerHPBar __instance, PlayerSpawner __0)
        {
            ...
            {
                if (!ResourceBarValueSettings.Get(ResourceBarValueSetting.TeammateResourceNumbers) || player == null || player.MaxHp <= 0f)
                    return string.Empty;
                string health = ResourceBarValueFormatter.HealthWithShield(player.IsDead ? 0f : player.Networkhp, player.MaxHp, player.Shield);
                string mana = ResourceBarValueFormatter.Mana(player.mp, player.MaxMp, player.reservedMp, ReserveLabel());

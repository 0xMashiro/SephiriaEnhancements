...
进入联机大厅后，靠近与根之退并排的**队伍规则**石碑，使用游戏原有的交互操作打开专用面板。房主编辑草稿并统一应用；应用修改会通过游戏聊天广播变更，进入探索后再广播一次最终规则，未安装 Mod 的队友也能接收这些普通聊天消息。广播使用房主的游戏语言。关闭不应用会丢弃草稿；开始探索后锁定，继续已有探索时保留存档中的规则。
面板自动识别当前队伍人数，直接调整敌人生命、伤害、数量与难度、奖励补给等细项，无需先选择预设。说明区并列显示原版人数效果与调整后数值，并提供适用的计算示例。恢复按钮明确显示当前队伍人数；人数变化时会更新显示并提示，应用只保存当前人数的数值调整。队员查看房主已应用的规则。
...
关闭 Mod 时隐藏下方设置分类，并保留已保存的选项；日志和问题反馈入口仍可使用。自定义联机规则可点击数值或使用确认操作打开输入框，数值项不再使用微调箭头。按 Enter 确认输入、Esc 取消，Tab / Shift+Tab 切换输入框与按钮；手柄可使用窗口内数字键盘。确认只更新本次修改，返回主面板后统一应用。主面板对照原版、已应用和本次修改，显示尚未应用的数量。输入框显示当前队伍人数、可提供的原版参考、范围和步长；清空输入可恢复该项原版行为。参考值仅对应设置控制的部分，不代表敌人的最终属性。

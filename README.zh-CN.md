...
- **战斗显示**：伤害、DPS、战后统计、队友伤害来源、训练假人统计、连续命中、资源数值和敌友轮廓。
...
- **外观选择**：更换外观，同时保留装扮的能力与初始装备。
- **阶段奖励**：可选地按当前存档的最高通关阶段自动补领遗漏奖励，默认关闭。
- **其他选项**：战斗伙伴、失败后重试、中途加入／重连，以及 **1–4 人**游戏规则。
...
| 询问奖励需求／商店代买 | 选中物品按 `L`，或中键点击；向下移出物品区域可进入交流按钮 |
...
共用的**情境操作**与自动施法暂停默认没有手柄绑定，可在改键设置中自行绑定；物品交流也可通过选中按钮并确认完成。

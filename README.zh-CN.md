...
关闭 Mod 时隐藏下方设置分类，并保留已保存的选项。自定义联机规则可点击数值或使用确认操作打开输入框，数值项不再使用微调箭头。按 Enter 确认输入、Esc 取消，Tab / Shift+Tab 切换输入框与按钮；手柄可使用窗口内数字键盘。确认只更新本次修改，返回主面板后统一应用。主面板对照原版、已应用和本次修改，显示尚未应用的数量。输入框显示当前队伍人数、可提供的原版参考、范围和步长；清空输入可恢复该项原版行为。参考值仅对应设置控制的部分，不代表敌人的最终属性。

...
namespace SephiriaEnhancements.DeveloperConsole.Integration
{
    internal static class DeveloperConsoleOptions
    {
        internal static void Inject(UI_OptionsPanel panel, UI_OptionBox_PartyMemberDamage template, Transform section)
        {
            if (panel.GetComponentInChildren<DeveloperConsoleOption>(true) == null)
            {
                CreateDeveloperConsoleRow(template, section);
            }
        }

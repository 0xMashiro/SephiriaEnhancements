...
namespace SephiriaEnhancements.Combat
{
    ...
    internal sealed class CombatInsightsController : MonoBehaviour
    {
        ...
        private const float HudHideHoldSeconds = 0.5f;
        ...
        private bool hudHiddenByUser, hudHideHoldHandled;
        private float hudHideHoldStartedAt = -1f;
        ...
        internal float EncounterElapsed => encounterActive
            ? Mathf.Max(0f, Time.unscaledTime - encounterStartedAt)
        ...
        private void Tick()
        {
            ...
            bool trackOrdinaryEncounters = statisticsEnabled &&
                displayPolicy != CombatInsightsDisplayPolicy.BossOnly;
            if (now >= nextSample)
            {
            ...
            HandleHudHideShortcut(contextAllowed, now);
            bool reportPresentationAvailable = contextAllowed &&
                !hudHiddenByUser && !encounterActive &&
        ...
        private void HandleHudHideShortcut(bool contextAllowed, float now)
        {
            ...
            bool canToggle = EnhancementsSettings.Enabled && contextAllowed &&
                (hudHiddenByUser || ViewMode != CombatInsightsViewMode.Hidden);
            if (!canToggle || !NativeInputActions.IsPressed(actions,
                    ModShortcuts.ToggleDamageStatistics,
                    rejectKeyboardModifiers: true))
            {
                hudHideHoldStartedAt = -1f;
                hudHideHoldHandled = false;
                return;
            }
            ...
            if (NativeInputActions.WasPressed(actions,
                    ...
                    rejectKeyboardModifiers: true))
            {
                hudHideHoldStartedAt = now;
                hudHideHoldHandled = false;
            }
            else if (!hudHideHoldHandled && hudHideHoldStartedAt >= 0f &&
                now - hudHideHoldStartedAt >= HudHideHoldSeconds)
            {
                hudHiddenByUser = !hudHiddenByUser;
                hudHideHoldHandled = true;
        ...
        private void SamplePlayers(float now, bool trackOrdinaryEncounters)
        {
            ...
            activeKeys.Clear();
            PlayerAvatar localAvatar = CombatManager.Instance?.CurrentPlayer ??
        ...
        private void EnsureAreaEncounter(EncounterScope scope, float now)
        {
            if (scope == null || bossEncounter.Active) return;
            if (scope.Kind == EncounterScopeKind.Ordinary &&
                runtimeKernel?.IsOrdinaryEncounterCleared(
                    scope.SourceInstanceId) == true)
                return;
            if (encounterActive && encounterScope != null && encounterScope.IsSame(scope)) return;
        ...
        {
            var players = new List<EncounterReportPlayerSnapshot>(4);
            foreach (PlayerDamageState state in states.Values)
            {
                ...
                players.Add(new EncounterReportPlayerSnapshot(state.Key,
                    state.Name, state.IsLocal, damage));
            }
            players.Sort(CompareReportPlayers);
            var damageTypes = new List<EncounterReportDamageTypeSnapshot>(
                damageByType.Count);
            foreach (KeyValuePair<EncounterDamageType, float> type in
                damageByType)
            {
                if (type.Value > 0f)
                    damageTypes.Add(new EncounterReportDamageTypeSnapshot(
                        type.Key, type.Value));
            }
            ...
            float now)
        {
            ...
            reportWindow.Start(now,
                EncounterReportPresentationPolicy.DisplaySeconds(report));
        }
        ...
        private static int CompareReportPlayers(
            EncounterReportPlayerSnapshot left,
            EncounterReportPlayerSnapshot right)
        {
        ...
        {
            if (StatisticsCaptureEnabled && owner != null)
            {
            ...
            float damage, EncounterDamageType damageType)
        {
            ...
            PlayerDamageState local = FindLocal();
            PlayerAvatar localAvatar = local?.Avatar ?? CombatManager.Instance?.CurrentPlayer ??
                GameCamera.Instance?.Observer;

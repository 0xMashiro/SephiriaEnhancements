...
进入联机大厅后，从多人菜单或设置中的**队伍规则**打开专用面板。房主编辑草稿并统一应用，关闭不应用会丢弃草稿；开始探索后锁定，继续已有探索时保留存档中的规则。
...
关闭 Mod 时隐藏下方设置分类，并保留已保存的选项。自定义联机规则可点击数值或使用确认操作打开输入框，左右箭头仍可逐步调整。输入框显示当前队伍人数、可提供的原版参考、范围和步长；清空输入可恢复该项原版行为。参考值仅对应设置控制的部分，不代表敌人的最终属性。
